...
void ggml_cuda_op_pad(ggml_backend_cuda_context & ctx, ggml_tensor * dst) {
    ...
    const size_t s00 = nb00 / type_size;
    const size_t s01 = nb01 / type_size;
    const size_t s02 = nb02 / type_size;
    const size_t s03 = nb03 / type_size;

    if (dst->type == GGML_TYPE_F16) {
        pad_cuda((const half *) src0->data, s00, s01, s02, s03, (half *) dst->data,
                 lp0, rp0, lp1, rp1, lp2, rp2, lp3, rp3,
                 dst->ne[0], dst->ne[1], dst->ne[2], dst->ne[3],
                 (bool) circular, stream);
    } else {
        pad_cuda((const float *) src0->data, s00, s01, s02, s03, (float *) dst->data,
                 lp0, rp0, lp1, rp1, lp2, rp2, lp3, rp3,
                 dst->ne[0], dst->ne[1], dst->ne[2], dst->ne[3],
                 (bool) circular, stream);
    }
