...
static bool ggml_cuda_qwen_fused_qkv_pack_name_matches(const ggml_tensor* dst) {
    ...
    return std::strstr(name, "qwen.") != nullptr ||
           ...
           std::strstr(name, "_q_attn_in") != nullptr ||
...
static bool ggml_cuda_wan_sp_triplet_qkv_send_pack_enabled() {
    static const bool enabled = [] {
        const char* env = std::getenv("ED_WAN_SP_TRIPLET_QKV_SEND_PACK");
        if (env == nullptr || env[0] == '\0') {
            return true;
        }
        return std::strcmp(env, "0") != 0 &&
...
static bool ggml_cuda_wan_sp_vec2_recv_unpack_enabled() {
    static const bool enabled = [] {
        const char* env = std::getenv("ED_WAN_SP_VEC2_RECV_UNPACK");
        if (env == nullptr || env[0] == '\0') {
            return true;
        }
        return std::strcmp(env, "0") != 0 &&
...
static bool ggml_cuda_qwen_ptr_aligned(const void* ptr, size_t alignment) {
...
static const char* ggml_cuda_qwen_fused_profile_mode_detail(int64_t mode) {
    switch (mode) {
        ...
        case 49:
            return "joint_qkv_send_pack";
        case 50:
            return "joint_qk_recv_pack";
        case 51:
        ...
        case 56:
            return "wan_qkv_roped_all_half_send_pack";
        case 57:
            return "wan_roped_all_half_q_recv_unpack";
        case 58:
            return "wan_roped_all_half_kv_recv_unpack";
        case 59:
            return "wan_head_to_seq_send_pack_f16";
        case 60:
            return "wan_head_to_seq_recv_unpack_f16";
        default:
            return "other";
    }
}
...
static __global__ void qwen_fused_scale_f32_to_f16_kernel(const float* __restrict__ src,
    ...
    for (int64_t linear = blockIdx.x * blockDim.x + threadIdx.x;
         linear < total;
         linear += static_cast<int64_t>(gridDim.x) * blockDim.x) {
        dst[linear] = __float2half_rn(src[linear] * scale);
    }
}

static __global__ void qwen_fused_scale_f16_to_f16_kernel(const half* __restrict__ src,
                                                          half* __restrict__ dst,
                                                          float scale,
                                                          int64_t total) {
    for (int64_t linear = blockIdx.x * blockDim.x + threadIdx.x;
         linear < total;
         linear += static_cast<int64_t>(gridDim.x) * blockDim.x) {
        dst[linear] = __float2half_rn(__half2float(src[linear]) * scale);
    }
}
...
static __global__ void qwen_fused_joint_qkv_send_pack_f32_kernel(const char* __restrict__ txt_q,
                                                                 const char* __restrict__ txt_k,
                                                                 const char* __restrict__ txt_v,
                                                                 const char* __restrict__ img_q,
                                                                 const char* __restrict__ img_k,
                                                                 ...
                                                                 int64_t head_dim,
                                                                 int64_t heads,
                                                                 int64_t txt_shard_sequence,
                                                                 int64_t img_shard_sequence,
                                                                 size_t txt_q_nb0,
                                                                 size_t txt_q_nb1,
                                                                 size_t txt_q_nb2,
                                                                 size_t txt_q_nb3,
    ...
    const int64_t shard_heads = heads / world_size;
    const int64_t total_head_dim = head_dim * 3;
    const int64_t txt_chunk = total_head_dim * shard_heads * txt_shard_sequence;
    const int64_t img_chunk = total_head_dim * shard_heads * img_shard_sequence;
    ...
    for (int64_t linear = blockIdx.x * blockDim.x + threadIdx.x;
        ...
        if (qkv_plane == 0) {
            ...
            nb0 = is_img ? img_q_nb0 : txt_q_nb0;
            nb1 = is_img ? img_q_nb1 : txt_q_nb1;
            nb2 = is_img ? img_q_nb2 : txt_q_nb2;
            nb3 = is_img ? img_q_nb3 : txt_q_nb3;
        } else if (qkv_plane == 1) {
            src = is_img ? img_k : txt_k;
            nb0 = is_img ? img_k_nb0 : txt_k_nb0;
            nb1 = is_img ? img_k_nb1 : txt_k_nb1;
            nb2 = is_img ? img_k_nb2 : txt_k_nb2;
            nb3 = is_img ? img_k_nb3 : txt_k_nb3;
        } else {
            src = is_img ? img_v : txt_v;
            nb0 = is_img ? img_v_nb0 : txt_v_nb0;
            nb1 = is_img ? img_v_nb1 : txt_v_nb1;
            nb2 = is_img ? img_v_nb2 : txt_v_nb2;
            nb3 = is_img ? img_v_nb3 : txt_v_nb3;
        }
...
static __global__ void qwen_fused_joint_qkv_send_pack_f32_triplet_kernel(
    const char* __restrict__ txt_q,
    const char* __restrict__ txt_k,
    const char* __restrict__ txt_v,
    const char* __restrict__ img_q,
    const char* __restrict__ img_k,
    ...
    int head_dim,
    int heads,
    int txt_shard_sequence,
    int img_shard_sequence,
    size_t txt_q_nb0,
    size_t txt_q_nb1,
    size_t txt_q_nb2,
    size_t txt_k_nb0,
    ...
    const int txt_chunk = total_head_dim * shard_heads * txt_shard_sequence;
    const int img_chunk = total_head_dim * shard_heads * img_shard_sequence;
    const int count_per_peer = txt_chunk + img_chunk;
    const int total_sequence = txt_shard_sequence + img_shard_sequence;
    ...
    for (int64_t linear = static_cast<int64_t>(blockIdx.x) * blockDim.x + threadIdx.x;
        ...
        const bool is_img = combined_seq >= txt_shard_sequence;
        const int seq = is_img ? combined_seq - txt_shard_sequence : combined_seq;
        ...
        const int64_t base =
            stream_base +
            d +
            static_cast<int64_t>(head_local) * total_head_dim +
            static_cast<int64_t>(seq) * total_head_dim * shard_heads;

        if (!is_img) {
            const char* q_ptr = txt_q + static_cast<int64_t>(d) * txt_q_nb0 +
                                static_cast<int64_t>(head) * txt_q_nb1 +
                                static_cast<int64_t>(seq) * txt_q_nb2;
            const char* k_ptr = txt_k + static_cast<int64_t>(d) * txt_k_nb0 +
                                static_cast<int64_t>(head) * txt_k_nb1 +
                                static_cast<int64_t>(seq) * txt_k_nb2;
            const char* v_ptr = txt_v + static_cast<int64_t>(d) * txt_v_nb0 +
                                static_cast<int64_t>(head) * txt_v_nb1 +
                                static_cast<int64_t>(seq) * txt_v_nb2;
            dst[base] = *reinterpret_cast<const float*>(q_ptr);
            dst[base + head_dim] = *reinterpret_cast<const float*>(k_ptr);
            dst[base + 2 * head_dim] = *reinterpret_cast<const float*>(v_ptr);
        } else {
            const char* q_ptr = img_q + static_cast<int64_t>(d) * img_q_nb0 +
                                static_cast<int64_t>(head) * img_q_nb1 +
                                static_cast<int64_t>(seq) * img_q_nb2;
            const char* k_ptr = img_k + static_cast<int64_t>(d) * img_k_nb0 +
                                static_cast<int64_t>(head) * img_k_nb1 +
                                static_cast<int64_t>(seq) * img_k_nb2;
            const char* v_ptr = img_v + static_cast<int64_t>(d) * img_v_nb0 +
                                static_cast<int64_t>(head) * img_v_nb1 +
                                static_cast<int64_t>(seq) * img_v_nb2;
            dst[base] = *reinterpret_cast<const float*>(q_ptr);
            dst[base + head_dim] = *reinterpret_cast<const float*>(k_ptr);
            dst[base + 2 * head_dim] = *reinterpret_cast<const float*>(v_ptr);
        }
...
static __device__ __forceinline__ uint32_t wan_roped_pair_to_half2_bits(const char* __restrict__ x,
                                                                        const char* __restrict__ pe,
                                                                        int64_t half,
                                                                        int64_t head,
                                                                        int64_t local_seq,
    ...
    const float x0 = *reinterpret_cast<const float*>(x + (2 * half) * x_nb0 + head * x_nb1 + local_seq * x_nb2 + 0 * x_nb3);
    const float x1 = *reinterpret_cast<const float*>(x + (2 * half + 1) * x_nb0 + head * x_nb1 + local_seq * x_nb2 + 0 * x_nb3);
    const float pe00 = *reinterpret_cast<const float*>(pe + 0 * pe_nb0 + half * pe_nb1 + global_seq * pe_nb2 + 0 * pe_nb3);
    const float pe01 = *reinterpret_cast<const float*>(pe + 0 * pe_nb0 + half * pe_nb1 + global_seq * pe_nb2 + 1 * pe_nb3);
    const float pe10 = *reinterpret_cast<const float*>(pe + 1 * pe_nb0 + half * pe_nb1 + global_seq * pe_nb2 + 0 * pe_nb3);
    const float pe11 = *reinterpret_cast<const float*>(pe + 1 * pe_nb0 + half * pe_nb1 + global_seq * pe_nb2 + 1 * pe_nb3);
    const float y0 = __fadd_rn(__fmul_rn(x0, pe00), __fmul_rn(x1, pe01));
    const float y1 = __fadd_rn(__fmul_rn(x0, pe10), __fmul_rn(x1, pe11));
    const uint32_t h0 = static_cast<uint32_t>(__half_as_ushort(__float2half_rn(y0)));
    const uint32_t h1 = static_cast<uint32_t>(__half_as_ushort(__float2half_rn(y1)));
    return h0 | (h1 << 16);
}

static __device__ __forceinline__ uint32_t wan_pack_roped_pair_with_pe_bits(float x0,
                                                                            float x1,
                                                                            float pe00,
                                                                            float pe01,
                                                                            float pe10,
...
static __global__ void wan_fused_qkv_roped_all_half_send_pack_kernel(const char* __restrict__ q,
                                                                     const char* __restrict__ k,
                                                                     const char* __restrict__ v,
                                                                     const char* __restrict__ pe,
                                                                     uint32_t* __restrict__ dst,
    ...
    for (int64_t linear = blockIdx.x * blockDim.x + threadIdx.x;
        ...
        const int64_t half_idx = d_all - plane * half_dim;
        const int64_t d0 = 2 * half_idx;
        const int64_t d1 = d0 + 1;
        if (plane == 0) {
            dst[linear] = wan_roped_pair_to_half2_bits(q,
                                                       pe,
                                                       half_idx,
                                                       head,
                                                       seq,
            ...
            dst[linear] = wan_roped_pair_to_half2_bits(k,
                                                       pe,
                                                       half_idx,
                                                       head,
                                                       seq,
            ...
            const float v0 = *reinterpret_cast<const float*>(v + d0 * v_nb0 + head * v_nb1 + seq * v_nb2 + 0 * v_nb3);
            const float v1 = *reinterpret_cast<const float*>(v + d1 * v_nb0 + head * v_nb1 + seq * v_nb2 + 0 * v_nb3);
            const uint32_t h0 = static_cast<uint32_t>(__half_as_ushort(__float2half_rn(v0)));
            const uint32_t h1 = static_cast<uint32_t>(__half_as_ushort(__float2half_rn(v1)));
            dst[linear] = h0 | (h1 << 16);
        }
    }
}

static __global__ void wan_fused_qkv_roped_all_half_send_pack_triplet_kernel(
    ...
    const int64_t shard_heads = heads / world_size;
    const int64_t packed_dim = half_dim * 3;
    const int64_t total_pairs = half_dim * shard_heads * shard_sequence * world_size;

    for (int64_t linear = blockIdx.x * blockDim.x + threadIdx.x;
         linear < total_pairs;
         linear += static_cast<int64_t>(gridDim.x) * blockDim.x) {
        int64_t rem = linear;
        const int64_t half_idx = rem % half_dim;
        rem /= half_dim;
        const int64_t head_local = rem % shard_heads;
        rem /= shard_heads;
        const int64_t seq = rem % shard_sequence;
        ...
        const int64_t group = head_local + shard_heads * (seq + shard_sequence * peer);
        const int64_t dst_base = half_idx + packed_dim * group;
        const int64_t d0 = 2 * half_idx;
        const int64_t d1 = d0 + 1;
        const float pe00 = *reinterpret_cast<const float*>(pe + 0 * pe_nb0 + half_idx * pe_nb1 + global_seq * pe_nb2 + 0 * pe_nb3);
        const float pe01 = *reinterpret_cast<const float*>(pe + 0 * pe_nb0 + half_idx * pe_nb1 + global_seq * pe_nb2 + 1 * pe_nb3);
        const float pe10 = *reinterpret_cast<const float*>(pe + 1 * pe_nb0 + half_idx * pe_nb1 + global_seq * pe_nb2 + 0 * pe_nb3);
        const float pe11 = *reinterpret_cast<const float*>(pe + 1 * pe_nb0 + half_idx * pe_nb1 + global_seq * pe_nb2 + 1 * pe_nb3);

        const float q0 = *reinterpret_cast<const float*>(q + d0 * q_nb0 + head * q_nb1 + seq * q_nb2 + 0 * q_nb3);
        const float q1 = *reinterpret_cast<const float*>(q + d1 * q_nb0 + head * q_nb1 + seq * q_nb2 + 0 * q_nb3);
        dst[dst_base] = wan_pack_roped_pair_with_pe_bits(q0, q1, pe00, pe01, pe10, pe11);

        const float k0 = *reinterpret_cast<const float*>(k + d0 * k_nb0 + head * k_nb1 + seq * k_nb2 + 0 * k_nb3);
        const float k1 = *reinterpret_cast<const float*>(k + d1 * k_nb0 + head * k_nb1 + seq * k_nb2 + 0 * k_nb3);
        dst[dst_base + half_dim] = wan_pack_roped_pair_with_pe_bits(k0, k1, pe00, pe01, pe10, pe11);

        const float v0 = *reinterpret_cast<const float*>(v + d0 * v_nb0 + head * v_nb1 + seq * v_nb2 + 0 * v_nb3);
        const float v1 = *reinterpret_cast<const float*>(v + d1 * v_nb0 + head * v_nb1 + seq * v_nb2 + 0 * v_nb3);
        const uint32_t h0 = static_cast<uint32_t>(__half_as_ushort(__float2half_rn(v0)));
        const uint32_t h1 = static_cast<uint32_t>(__half_as_ushort(__float2half_rn(v1)));
        dst[dst_base + 2 * half_dim] = h0 | (h1 << 16);
    }
}
...
static __global__ void wan_fused_attn_head_to_seq_send_pack_f16_kernel(const char* __restrict__ attn,
                                                                       half* __restrict__ dst,
                                                                       int64_t world_size,
                                                                       int64_t head_dim,
                                                                       int64_t shard_heads,
    ...
    for (int64_t linear = blockIdx.x * blockDim.x + threadIdx.x;
        ...
        const char* src = attn +
                          ...
                          seq * attn_nb2 +
                          0 * attn_nb3;
        dst[linear] = attn_is_f16 ?
                          *reinterpret_cast<const half*>(src) :
                          __float2half(*reinterpret_cast<const float*>(src));
    }
}

static __global__ void wan_fused_attn_head_to_seq_send_pack_f32_to_f16_h2_kernel(
    ...
    const int64_t shard_sequence = sequence / world_size;
    const int64_t count_per_peer2 = head_dim2 * shard_heads * shard_sequence;
    ...
    for (int64_t linear = blockIdx.x * blockDim.x + threadIdx.x;
        ...
        const char* src = attn +
                          ...
                          seq * attn_nb2 +
                          0 * attn_nb3;
        dst[linear] = __float22half2_rn(*reinterpret_cast<const float2*>(src));
    }
}

static __global__ void wan_fused_attn_head_to_seq_send_pack_f16_h2_kernel(const char* __restrict__ attn,
                                                                          half2* __restrict__ dst,
                                                                          int64_t world_size,
                                                                          int64_t head_dim,
                                                                          int64_t shard_heads,
    ...
    for (int64_t linear = blockIdx.x * blockDim.x + threadIdx.x;
        ...
        const char* src = attn +
                          ...
                          seq * attn_nb2 +
                          0 * attn_nb3;
        dst[linear] = *reinterpret_cast<const half2*>(src);
    }
}
...
static __global__ void wan_fused_attn_head_to_seq_recv_unpack_f16_kernel(const half* __restrict__ recv_flat,
                                                                         float* __restrict__ dst,
                                                                         int64_t world_size,
                                                                         int64_t head_dim,
                                                                         int64_t heads,
...
static __global__ void wan_fused_attn_head_to_seq_recv_unpack_f16_h2_kernel(const half* __restrict__ recv_flat,
                                                                            float* __restrict__ dst,
                                                                            int64_t world_size,
                                                                            int64_t head_dim,
                                                                            int64_t heads,
    ...
    const int64_t head_dim2 = head_dim / 2;
    const int64_t count_per_peer = head_dim * shard_heads * shard_sequence;
    const int64_t total = head_dim2 * heads * shard_sequence;
    ...
    for (int64_t linear = blockIdx.x * blockDim.x + threadIdx.x;
        ...
        const int64_t src_idx = src_peer * count_per_peer +
                                ...
                                local_head * head_dim +
                                local_seq * head_dim * shard_heads;
        const int64_t dst_idx = 2 * d2 + head_dim * (head + heads * local_seq);
...
static __global__ void wan_fused_roped_all_half_recv_unpack_kernel(const uint32_t* __restrict__ recv_flat,
                                                                   half* __restrict__ dst,
                                                                   int64_t world_size,
                                                                   int64_t first_plane,
                                                                   int64_t head_dim,
    ...
    const int64_t half_dim   = head_dim / 2;
    const int64_t packed_dim = half_dim * 3;
    const int64_t total      = head_dim * sequence * shard_heads * out_planes;
    ...
    for (int64_t linear = blockIdx.x * blockDim.x + threadIdx.x;
        ...
        const int64_t base_idx = head * packed_dim +
                                 local_seq * packed_dim * shard_heads +
                                 peer * packed_dim * shard_heads * shard_sequence;
        const int64_t src_idx = base_idx + plane * half_dim + d / 2;
        const uint32_t packed = recv_flat[src_idx];
        const uint16_t bits = (d & 1) == 0 ?
                                  static_cast<uint16_t>(packed & 0xffffu) :
                                  static_cast<uint16_t>(packed >> 16);
...
static __global__ void wan_fused_roped_all_half_recv_unpack_h2_kernel(const uint32_t* __restrict__ recv_flat,
                                                                      uint32_t* __restrict__ dst,
                                                                      int64_t world_size,
                                                                      int64_t first_plane,
                                                                      int64_t head_dim,
    ...
    const int64_t half_dim   = head_dim / 2;
    const int64_t packed_dim = half_dim * 3;
    const int64_t total      = half_dim * sequence * shard_heads * out_planes;
    ...
    for (int64_t linear = blockIdx.x * blockDim.x + threadIdx.x;
        ...
        const int64_t base_idx = head * packed_dim +
                                 local_seq * packed_dim * shard_heads +
                                 peer * packed_dim * shard_heads * shard_sequence;
        const int64_t src_idx = base_idx + plane * half_dim + half;
        const int64_t dst_idx_h2 = half +
                                   half_dim * (seq + sequence * (head + shard_heads * rem));
        dst[dst_idx_h2] = recv_flat[src_idx];
    }
}

static __global__ void wan_fused_roped_all_half_recv_unpack_h2_vec2_kernel(const uint32_t* __restrict__ recv_flat,
                                                                           uint32_t* __restrict__ dst,
                                                                           int64_t world_size,
                                                                           int64_t first_plane,
                                                                           int64_t head_dim,
    ...
    const int64_t sequence   = shard_sequence * world_size;
    const int64_t half_dim   = head_dim / 2;
    const int64_t half_dim2  = half_dim / 2;
    const int64_t packed_dim = half_dim * 3;
    const int64_t total      = half_dim2 * sequence * shard_heads * out_planes;

    for (int64_t linear = blockIdx.x * blockDim.x + threadIdx.x;
         linear < total;
         linear += static_cast<int64_t>(gridDim.x) * blockDim.x) {
        int64_t rem            = linear;
        const int64_t half2    = rem % half_dim2;
        rem /= half_dim2;
        const int64_t seq      = rem % sequence;
        rem /= sequence;
        const int64_t head     = rem % shard_heads;
        ...
        const int64_t dst_idx_h2 = half +
                                   half_dim * (seq + sequence * (head + shard_heads * rem));
        *reinterpret_cast<uint2*>(dst + dst_idx_h2) =
            *reinterpret_cast<const uint2*>(recv_flat + src_idx);
    }
}

static __global__ void wan_fused_roped_all_half_recv_unpack_h2_vec4_kernel(const uint32_t* __restrict__ recv_flat,
                                                                           uint32_t* __restrict__ dst,
                                                                           int64_t world_size,
                                                                           int64_t first_plane,
                                                                           int64_t head_dim,
    ...
    const int64_t sequence   = shard_sequence * world_size;
    const int64_t half_dim   = head_dim / 2;
    const int64_t half_dim4  = half_dim / 4;
    const int64_t packed_dim = half_dim * 3;
    const int64_t total      = half_dim4 * sequence * shard_heads * out_planes;

    for (int64_t linear = blockIdx.x * blockDim.x + threadIdx.x;
         linear < total;
         linear += static_cast<int64_t>(gridDim.x) * blockDim.x) {
        int64_t rem            = linear;
        const int64_t half4    = rem % half_dim4;
        rem /= half_dim4;
        const int64_t seq      = rem % sequence;
        rem /= sequence;
        const int64_t head     = rem % shard_heads;
        ...
        const int64_t dst_idx_h2 = half +
                                   half_dim * (seq + sequence * (head + shard_heads * rem));
        *reinterpret_cast<uint4*>(dst + dst_idx_h2) =
            *reinterpret_cast<const uint4*>(recv_flat + src_idx);
    }
}
...
static __global__ void qwen_fused_attn_head_to_seq_send_pack_f32_vec4_kernel(
    ...
    const int txt_chunk_vec = vec_dim * shard_heads * txt_shard_seq;
    const int img_chunk_vec = vec_dim * shard_heads * img_shard_seq;
    const int count_per_peer_vec = txt_chunk_vec + img_chunk_vec;
    const int64_t total = static_cast<int64_t>(count_per_peer_vec) * world_size;

    for (int64_t linear = static_cast<int64_t>(blockIdx.x) * blockDim.x + threadIdx.x;
         linear < total;
         linear += static_cast<int64_t>(gridDim.x) * blockDim.x) {
        int64_t rem = linear;
        const int peer = static_cast<int>(rem / count_per_peer_vec);
        rem -= static_cast<int64_t>(peer) * count_per_peer_vec;
        const bool is_img = rem >= txt_chunk_vec;
        if (is_img) {
            rem -= txt_chunk_vec;
        }
        ...
        if (stream_tok < stream_real_seq) {
            ...
            const char* src =
                reinterpret_cast<const char*>(attn) +
                static_cast<int64_t>(vec * 4) * sizeof(float) +
                static_cast<int64_t>(head) * attn_nb1 +
                static_cast<int64_t>(total_tok) * attn_nb2;
            value = *reinterpret_cast<const float4*>(src);
        }
        *reinterpret_cast<float4*>(dst + linear * 4) = value;
    }
}
...
static __global__ void qwen_fused_attn_head_to_seq_recv_unpack_f32_vec4_kernel(
    const float* __restrict__ recv_flat,
    float* __restrict__ dst,
    int stream_index,
    int txt_padded_seq,
    int img_padded_seq,
    ...
    const int txt_shard_seq = txt_padded_seq / world_size;
    const int img_shard_seq = img_padded_seq / world_size;
    const int out_shard_seq = stream_index == 0 ? txt_shard_seq : img_shard_seq;
    const int txt_chunk = head_dim * shard_heads * txt_shard_seq;
    const int img_chunk = head_dim * shard_heads * img_shard_seq;
    const int count_per_peer = txt_chunk + img_chunk;
    const int stream_offset = stream_index == 0 ? 0 : txt_chunk;
    const int64_t total = static_cast<int64_t>(vec_dim) * heads * out_shard_seq;
    ...
    for (int64_t linear = static_cast<int64_t>(blockIdx.x) * blockDim.x + threadIdx.x;
        ...
        const int64_t dst_idx =
            d +
            static_cast<int64_t>(head_dim) * (head + heads * local_tok);
        *reinterpret_cast<float4*>(dst + dst_idx) =
            *reinterpret_cast<const float4*>(recv_flat + src_idx);
    }
}
...
static __global__ void qwen_fused_attn_head_to_seq_send_pack_f32_to_f16_h2_kernel(
    const char* __restrict__ attn,
    half2* __restrict__ dst,
    int64_t txt_real_seq,
    int64_t img_real_seq,
    ...
    for (int64_t linear = blockIdx.x * blockDim.x + threadIdx.x;
        ...
        if (stream_tok < stream_real_seq) {
            ...
            const char* src = attn +
                              ...
                              total_tok * attn_nb2 +
                              0 * attn_nb3;
            value = __float22half2_rn(*reinterpret_cast<const float2*>(src));
        }
        dst[linear] = value;
    }
}
...
static __global__ void qwen_fused_attn_head_to_seq_recv_unpack_f16_h2_kernel(
...
static __device__ __forceinline__ int64_t qwen_joint_qkv_recv_src_index(int64_t d_all,
                                                                        int64_t seq,
                                                                        int64_t head,
                                                                        int64_t stream_index,
                                                                        int64_t head_dim,
...
static __global__ void qwen_fused_qk_pack_from_joint_recv_f32_kernel(const char* __restrict__ recv_flat,
                                                                     float* __restrict__ dst,
                                                                     int64_t txt_real_seq,
                                                                     int64_t img_real_seq,
                                                                     int64_t txt_padded_seq,
    ...
    for (int64_t linear = blockIdx.x * blockDim.x + threadIdx.x;
        ...
        const int64_t part = plane % 2;
        const int64_t src_d = qkv_plane * head_dim + part + 2 * half;
        const int64_t src_idx = qwen_joint_qkv_recv_src_index(src_d,
                                                              src_t,
                                                              head,
                                                              is_txt ? 0 : 1,
                                                              head_dim,
...
static __global__ void qwen_fused_qk_pack_from_joint_recv_f16_kernel(const half* __restrict__ recv_flat,
                                                                     float* __restrict__ dst,
                                                                     int64_t txt_real_seq,
                                                                     int64_t img_real_seq,
                                                                     int64_t txt_padded_seq,
...
static __global__ void qwen_fused_qk_pack_from_joint_recv_f16_pair_kernel(const half* __restrict__ recv_flat,
                                                                          float* __restrict__ dst,
                                                                          int txt_real_seq,
                                                                          int img_real_seq,
                                                                          int txt_padded_seq,
                                                                          ...
                                                                          int heads,
                                                                          int head_dim) {
    const int seq_total = txt_real_seq + img_real_seq;
    const int qk_half_dim = head_dim / 2;
    const int plane_elems = qk_half_dim * seq_total * heads;
    const int txt_shard_seq = txt_padded_seq / world_size;
    const int img_shard_seq = img_padded_seq / world_size;
    const int total_head_dim = head_dim * 3;
    const int txt_chunk = total_head_dim * heads * txt_shard_seq;
    const int img_chunk = total_head_dim * heads * img_shard_seq;
    const int count_per_peer = txt_chunk + img_chunk;
    const int64_t total = static_cast<int64_t>(qk_half_dim) * seq_total * heads;

    for (int64_t linear = static_cast<int64_t>(blockIdx.x) * blockDim.x + threadIdx.x;
         linear < total;
         linear += static_cast<int64_t>(gridDim.x) * blockDim.x) {
        int64_t rem = linear;
        const int half = static_cast<int>(rem % qk_half_dim);
        rem /= qk_half_dim;
        const int tok = static_cast<int>(rem % seq_total);
        rem /= seq_total;
        const int head = static_cast<int>(rem);

        const bool is_img = tok >= txt_real_seq;
        const int src_t = is_img ? tok - txt_real_seq : tok;
        const int stream_shard_seq = is_img ? img_shard_seq : txt_shard_seq;
        const int peer = src_t / stream_shard_seq;
        const int local_seq = src_t - peer * stream_shard_seq;
        const int stream_offset = is_img ? txt_chunk : 0;
        const int64_t src_base =
            static_cast<int64_t>(peer) * count_per_peer +
            stream_offset +
            static_cast<int64_t>(head) * total_head_dim +
            static_cast<int64_t>(local_seq) * total_head_dim * heads +
            2 * half;
        const half2 q_pair = *reinterpret_cast<const half2*>(recv_flat + src_base);
        const half2 k_pair = *reinterpret_cast<const half2*>(recv_flat + src_base + head_dim);
        const float2 q_float = __half22float2(q_pair);
        const float2 k_float = __half22float2(k_pair);

        const int64_t dst_base =
            half +
            static_cast<int64_t>(qk_half_dim) * (tok + seq_total * head);
        dst[dst_base] = q_float.x;
        dst[dst_base + plane_elems] = q_float.y;
        dst[dst_base + 2 * plane_elems] = k_float.x;
        dst[dst_base + 3 * plane_elems] = k_float.y;
    }
}

static __global__ void qwen_fused_qk_pack_from_joint_recv_f32_quad_kernel(const float* __restrict__ recv_flat,
                                                                          float* __restrict__ dst,
                                                                          int txt_real_seq,
                                                                          int img_real_seq,
                                                                          int txt_padded_seq,
    ...
    for (int64_t linear = static_cast<int64_t>(blockIdx.x) * blockDim.x + threadIdx.x;
        ...
        const int64_t src_base =
            ...
            static_cast<int64_t>(local_seq) * total_head_dim * heads +
            2 * half;
        const float2 q_pair = *reinterpret_cast<const float2*>(recv_flat + src_base);
        const float2 k_pair = *reinterpret_cast<const float2*>(recv_flat + src_base + head_dim);

        const int64_t dst_base =
            half +
            static_cast<int64_t>(qk_half_dim) * (tok + seq_total * head);
        dst[dst_base] = q_pair.x;
        dst[dst_base + plane_elems] = q_pair.y;
        dst[dst_base + 2 * plane_elems] = k_pair.x;
        dst[dst_base + 3 * plane_elems] = k_pair.y;
    }
}

static __global__ void qwen_fused_v_pack_from_joint_recv_f32_kernel(const char* __restrict__ recv_flat,
                                                                    float* __restrict__ dst,
                                                                    int64_t txt_real_seq,
                                                                    int64_t img_real_seq,
                                                                    int64_t txt_padded_seq,
...
static __global__ void qwen_fused_v_pack_from_joint_recv_f16_kernel(const half* __restrict__ recv_flat,
                                                                    float* __restrict__ dst,
                                                                    int64_t txt_real_seq,
                                                                    int64_t img_real_seq,
                                                                    int64_t txt_padded_seq,
...
static __global__ void qwen_fused_v_pack_from_joint_recv_f16_pair_kernel(const half* __restrict__ recv_flat,
                                                                         float* __restrict__ dst,
                                                                         int txt_real_seq,
                                                                         int img_real_seq,
                                                                         int txt_padded_seq,
    ...
    const int img_chunk = total_head_dim * heads * img_shard_seq;
    const int count_per_peer = txt_chunk + img_chunk;
    const int64_t total = static_cast<int64_t>(half_dim) * seq_total * heads;

    for (int64_t linear = static_cast<int64_t>(blockIdx.x) * blockDim.x + threadIdx.x;
         linear < total;
         linear += static_cast<int64_t>(gridDim.x) * blockDim.x) {
        int64_t rem = linear;
        const int half = static_cast<int>(rem % half_dim);
        rem /= half_dim;
        const int tok = static_cast<int>(rem % seq_total);
        rem /= seq_total;
        const int head = static_cast<int>(rem);
        ...
        const int64_t src_base =
            ...
            static_cast<int64_t>(head) * total_head_dim +
            static_cast<int64_t>(local_seq) * total_head_dim * heads;
        const float2 value = __half22float2(*reinterpret_cast<const half2*>(recv_flat + src_base));
        const int64_t dst_base =
            d +
            static_cast<int64_t>(head_dim) * (tok + seq_total * head);
        dst[dst_base] = value.x;
        dst[dst_base + 1] = value.y;
    }
}

static __global__ void qwen_fused_v_pack_from_joint_recv_f32_vec4_kernel(const float* __restrict__ recv_flat,
                                                                         float* __restrict__ dst,
                                                                         int txt_real_seq,
                                                                         int img_real_seq,
                                                                         int txt_padded_seq,
    ...
    const int img_chunk = total_head_dim * heads * img_shard_seq;
    const int count_per_peer = txt_chunk + img_chunk;
    const int64_t total = static_cast<int64_t>(vec_dim) * seq_total * heads;
    ...
    for (int64_t linear = static_cast<int64_t>(blockIdx.x) * blockDim.x + threadIdx.x;
        ...
        const int64_t dst_base =
            d +
            static_cast<int64_t>(head_dim) * (tok + seq_total * head);
        *reinterpret_cast<float4*>(dst + dst_base) =
            *reinterpret_cast<const float4*>(recv_flat + src_base);
    }
}
...
void ggml_cuda_op_qwen_fused_qkv_pack(ggml_backend_cuda_context& ctx, ggml_tensor* dst) {
    ...
    if (qwen_params->mode == 52) {
        ...
        if (src->type == GGML_TYPE_F32) {
            qwen_fused_scale_f32_to_f16_kernel<<<blocks, threads, 0, stream>>>(
                static_cast<const float*>(src->data),
                static_cast<half*>(dst->data),
                qwen_params->scale,
                total);
        } else {
            qwen_fused_scale_f16_to_f16_kernel<<<blocks, threads, 0, stream>>>(
                static_cast<const half*>(src->data),
                static_cast<half*>(dst->data),
                qwen_params->scale,
                total);
        }
    ...
    if (qwen_params->mode == 7) {
        ...
        if (use_vec4) {
            const int64_t total_vec4 = total / 4;
            const int vec4_blocks = static_cast<int>(std::min<int64_t>((total_vec4 + threads - 1) / threads, 65535));
            qwen_fused_attn_head_to_seq_send_pack_f32_vec4_kernel<<<vec4_blocks, threads, 0, stream>>>(
    ...
    if (qwen_params->mode == 8) {
        ...
        if (use_vec4) {
            const int64_t total_vec4 = total / 4;
            const int vec4_blocks = static_cast<int>(std::min<int64_t>((total_vec4 + threads - 1) / threads, 65535));
            qwen_fused_attn_head_to_seq_recv_unpack_f32_vec4_kernel<<<vec4_blocks, threads, 0, stream>>>(
                static_cast<const float*>(recv_flat->data),
                static_cast<float*>(dst->data),
                static_cast<int>(qwen_params->stream_index),
                static_cast<int>(qwen_params->txt_padded_seq),
                static_cast<int>(qwen_params->img_padded_seq),
                static_cast<int>(qwen_params->world_size),
                static_cast<int>(dst->ne[0]),
    ...
    if (qwen_params->mode == 13) {
        ...
        const bool use_f16_h2 = attn->type == GGML_TYPE_F16 &&
                                attn->nb[0] == sizeof(half) &&
                                (attn->ne[0] % 2) == 0 &&
                                ggml_cuda_qwen_ptr_aligned(attn->data, alignof(half2)) &&
                                ggml_cuda_qwen_ptr_aligned(dst->data, alignof(half2)) &&
                                (attn->nb[1] % alignof(half2)) == 0 &&
                                (attn->nb[2] % alignof(half2)) == 0;
        const bool use_f32_to_f16_h2 = attn->type == GGML_TYPE_F32 &&
                                       attn->nb[0] == sizeof(float) &&
                                       (attn->ne[0] % 2) == 0 &&
                                       ggml_cuda_qwen_ptr_aligned(attn->data, alignof(float2)) &&
                                       ggml_cuda_qwen_ptr_aligned(dst->data, alignof(half2)) &&
        ...
        if (use_f32_to_f16_h4) {
            ...
            qwen_fused_attn_head_to_seq_send_pack_f16_h2_kernel<<<blocks, threads, 0, stream>>>(
                ...
                attn->ne[1],
                attn->nb[0], attn->nb[1], attn->nb[2], attn->nb[3]);
        } else if (use_f32_to_f16_h2) {
            qwen_fused_attn_head_to_seq_send_pack_f32_to_f16_h2_kernel<<<blocks, threads, 0, stream>>>(
                static_cast<const char*>(attn->data),
                reinterpret_cast<half2*>(dst->data),
                qwen_params->txt_real_seq,
                qwen_params->img_real_seq,
    ...
    if (qwen_params->mode == 14) {
        ...
        const bool profile = ggml_cuda_qwen_fused_profile_begin(stream, &profile_start, &profile_stop);
        if (use_h2) {
            qwen_fused_attn_head_to_seq_recv_unpack_f16_h2_kernel<<<blocks, threads, 0, stream>>>(
    ...
    if (qwen_params->mode == 37 || qwen_params->mode == 56) {
        GGML_ASSERT(dst->type == GGML_TYPE_F32);
        const ggml_tensor* q = dst->src[0];
        const ggml_tensor* k = dst->src[1];
        const ggml_tensor* v = dst->src[2];
        ...
        const int64_t packed_dim = qwen_params->mode == 56 ? q->ne[0] * 3 / 2 : q->ne[0] * 2;
        const int64_t shard_heads = q->ne[1] / world_size;
        GGML_ASSERT(dst->ne[0] == packed_dim * shard_heads * q->ne[2] * world_size);
        ...
        cudaEvent_t profile_stop = nullptr;
        const bool profile = ggml_cuda_qwen_fused_profile_begin(stream, &profile_start, &profile_stop);
        if (qwen_params->mode == 56) {
            const bool use_triplet = ggml_cuda_wan_sp_triplet_qkv_send_pack_enabled();
            const int64_t triplet_total = (q->ne[0] / 2) * shard_heads * q->ne[2] * world_size;
            const int triplet_blocks =
                static_cast<int>(std::min<int64_t>((triplet_total + threads - 1) / threads, 65535));
            if (use_triplet) {
                wan_fused_qkv_roped_all_half_send_pack_triplet_kernel<<<triplet_blocks, threads, 0, stream>>>(
                    static_cast<const char*>(q->data),
                    static_cast<const char*>(k->data),
                    static_cast<const char*>(v->data),
                ...
                wan_fused_qkv_roped_all_half_send_pack_kernel<<<blocks, threads, 0, stream>>>(
                    static_cast<const char*>(q->data),
                    static_cast<const char*>(k->data),
                    static_cast<const char*>(v->data),
                    static_cast<const char*>(pe->data),
    ...
    if (qwen_params->mode == 49) {
        ...
        GGML_ASSERT(txt_q && txt_k && txt_v && img_q && img_k && img_v);
        ...
        GGML_ASSERT(img_q->ne[0] == txt_q->ne[0] && img_k->ne[0] == txt_q->ne[0] && img_v->ne[0] == txt_q->ne[0]);
        GGML_ASSERT(txt_k->ne[1] == txt_q->ne[1] && txt_v->ne[1] == txt_q->ne[1]);
        GGML_ASSERT(img_q->ne[1] == txt_q->ne[1] && img_k->ne[1] == txt_q->ne[1] && img_v->ne[1] == txt_q->ne[1]);
        ...
        const int64_t max_i32 = static_cast<int64_t>(std::numeric_limits<int>::max());
        const int64_t triplet_total =
            txt_q->ne[0] * (txt_q->ne[1] / world_size) * (txt_q->ne[2] + img_q->ne[2]) * world_size;
        const bool use_triplet =
            txt_q->ne[0] <= max_i32 / 3 &&
            txt_q->ne[1] <= max_i32 &&
            txt_q->ne[2] <= max_i32 &&
            img_q->ne[2] <= max_i32 &&
            world_size <= max_i32 &&
            total <= max_i32 &&
            triplet_total <= max_i32;
        if (use_triplet) {
            const int triplet_blocks = static_cast<int>(std::min<int64_t>((triplet_total + threads - 1) / threads, 65535));
            qwen_fused_joint_qkv_send_pack_f32_triplet_kernel<<<triplet_blocks, threads, 0, stream>>>(
                ...
                static_cast<float*>(dst->data),
                static_cast<int>(world_size),
                static_cast<int>(txt_q->ne[0]),
                static_cast<int>(txt_q->ne[1]),
                static_cast<int>(txt_q->ne[2]),
                static_cast<int>(img_q->ne[2]),
                txt_q->nb[0], txt_q->nb[1], txt_q->nb[2],
                txt_k->nb[0], txt_k->nb[1], txt_k->nb[2],
                txt_v->nb[0], txt_v->nb[1], txt_v->nb[2],
                img_q->nb[0], img_q->nb[1], img_q->nb[2],
                img_k->nb[0], img_k->nb[1], img_k->nb[2],
                img_v->nb[0], img_v->nb[1], img_v->nb[2]);
        } else {
            qwen_fused_joint_qkv_send_pack_f32_kernel<<<blocks, threads, 0, stream>>>(
                static_cast<const char*>(txt_q->data),
                static_cast<const char*>(txt_k->data),
                static_cast<const char*>(txt_v->data),
                static_cast<const char*>(img_q->data),
    ...
    if (qwen_params->mode == 50 || qwen_params->mode == 51) {
        GGML_ASSERT(dst->type == GGML_TYPE_F32);
        const ggml_tensor* recv_flat = dst->src[0];
        GGML_ASSERT(recv_flat);
        ...
        const int64_t head_dim = qwen_params->mode == 50 ? dst->ne[0] * 2 : dst->ne[0];
        const int64_t heads = dst->ne[2];
        const int64_t txt_shard_seq = qwen_params->txt_padded_seq / qwen_params->world_size;
        const int64_t img_shard_seq = qwen_params->img_padded_seq / qwen_params->world_size;
        const int64_t expected = 3 * head_dim * heads * (txt_shard_seq + img_shard_seq) * qwen_params->world_size;
        GGML_ASSERT(recv_flat->ne[0] == expected);
        GGML_ASSERT((qwen_params->mode == 50 && dst->ne[3] == 4) ||
                    (qwen_params->mode == 51 && dst->ne[3] == 1));
        ...
        const bool use_i32 =
            ...
            expected <= max_i32 &&
            total <= max_i32;
        if (qwen_params->mode == 50) {
            const bool use_quad =
                recv_flat->type == GGML_TYPE_F32 &&
                use_i32 &&
                (head_dim % 2) == 0 &&
                (dst->nb[0] == sizeof(float)) &&
                ggml_cuda_qwen_ptr_aligned(recv_flat->data, alignof(float2)) &&
                ggml_cuda_qwen_ptr_aligned(dst->data, alignof(float2));
            if (use_quad) {
                const int64_t quad_total = (head_dim / 2) * (qwen_params->txt_real_seq + qwen_params->img_real_seq) * heads;
                const int quad_blocks = static_cast<int>(std::min<int64_t>((quad_total + threads - 1) / threads, 65535));
                qwen_fused_qk_pack_from_joint_recv_f32_quad_kernel<<<quad_blocks, threads, 0, stream>>>(
                    static_cast<const float*>(recv_flat->data),
                    static_cast<float*>(dst->data),
                    static_cast<int>(qwen_params->txt_real_seq),
                    static_cast<int>(qwen_params->img_real_seq),
                ...
                if (recv_flat->type == GGML_TYPE_F16) {
                    const bool use_pair =
                        ...
                        ggml_cuda_qwen_ptr_aligned(recv_flat->data, alignof(half2));
                    if (use_pair) {
                        const int64_t pair_total = (head_dim / 2) * (qwen_params->txt_real_seq + qwen_params->img_real_seq) * heads;
                        const int pair_blocks = static_cast<int>(std::min<int64_t>((pair_total + threads - 1) / threads, 65535));
                        qwen_fused_qk_pack_from_joint_recv_f16_pair_kernel<<<pair_blocks, threads, 0, stream>>>(
                            static_cast<const half*>(recv_flat->data),
                            static_cast<float*>(dst->data),
                            static_cast<int>(qwen_params->txt_real_seq),
                            static_cast<int>(qwen_params->img_real_seq),
                        ...
                        qwen_fused_qk_pack_from_joint_recv_f16_kernel<<<blocks, threads, 0, stream>>>(
                            static_cast<const half*>(recv_flat->data),
                            static_cast<float*>(dst->data),
                            qwen_params->txt_real_seq,
                            qwen_params->img_real_seq,
                    ...
                    qwen_fused_qk_pack_from_joint_recv_f32_kernel<<<blocks, threads, 0, stream>>>(
                        static_cast<const char*>(recv_flat->data),
                        static_cast<float*>(dst->data),
                        qwen_params->txt_real_seq,
                        qwen_params->img_real_seq,
            ...
            const bool use_vec4 =
                ...
                ggml_cuda_qwen_ptr_aligned(dst->data, alignof(float4));
            if (use_vec4) {
                const int64_t vec4_total = (head_dim / 4) * (qwen_params->txt_real_seq + qwen_params->img_real_seq) * heads;
                const int vec4_blocks = static_cast<int>(std::min<int64_t>((vec4_total + threads - 1) / threads, 65535));
                qwen_fused_v_pack_from_joint_recv_f32_vec4_kernel<<<vec4_blocks, threads, 0, stream>>>(
                    static_cast<const float*>(recv_flat->data),
                    static_cast<float*>(dst->data),
                    static_cast<int>(qwen_params->txt_real_seq),
                    static_cast<int>(qwen_params->img_real_seq),
                ...
                if (recv_flat->type == GGML_TYPE_F16) {
                    ...
                    if (use_pair) {
                        const int64_t pair_total = (head_dim / 2) * (qwen_params->txt_real_seq + qwen_params->img_real_seq) * heads;
                        const int pair_blocks = static_cast<int>(std::min<int64_t>((pair_total + threads - 1) / threads, 65535));
                        qwen_fused_v_pack_from_joint_recv_f16_pair_kernel<<<pair_blocks, threads, 0, stream>>>(
                            static_cast<const half*>(recv_flat->data),
                            static_cast<float*>(dst->data),
                            static_cast<int>(qwen_params->txt_real_seq),
                            static_cast<int>(qwen_params->img_real_seq),
                        ...
                        qwen_fused_v_pack_from_joint_recv_f16_kernel<<<blocks, threads, 0, stream>>>(
                            static_cast<const half*>(recv_flat->data),
                            static_cast<float*>(dst->data),
                            qwen_params->txt_real_seq,
                            qwen_params->img_real_seq,
                    ...
                    qwen_fused_v_pack_from_joint_recv_f32_kernel<<<blocks, threads, 0, stream>>>(
                        static_cast<const char*>(recv_flat->data),
                        static_cast<float*>(dst->data),
                        qwen_params->txt_real_seq,
                        qwen_params->img_real_seq,
    ...
    if (qwen_params->mode == 57 || qwen_params->mode == 58) {
        GGML_ASSERT(dst->type == GGML_TYPE_F16);
        const ggml_tensor* recv_flat = dst->src[0];
        GGML_ASSERT(recv_flat);
        GGML_ASSERT(recv_flat->type == GGML_TYPE_F32);
        const int64_t world_size = qwen_params->txt_real_seq;
        GGML_ASSERT(world_size > 0);
        GGML_ASSERT(qwen_params->mode == 57 ? dst->ne[3] == 1 : dst->ne[3] == 2);
        GGML_ASSERT(dst->ne[1] % world_size == 0);
        const int64_t head_dim = dst->ne[0];
        const int64_t shard_sequence = dst->ne[1] / world_size;
        const int64_t shard_heads = dst->ne[2];
        ...
        const bool use_h2 = dst->nb[0] == sizeof(half) &&
                            (head_dim % 2) == 0 &&
                            ggml_cuda_qwen_ptr_aligned(recv_flat->data, alignof(uint32_t)) &&
                            ggml_cuda_qwen_ptr_aligned(dst->data, alignof(uint32_t));
        const bool use_vec2 = use_h2 &&
                              ggml_cuda_wan_sp_vec2_recv_unpack_enabled() &&
                              (head_dim % 4) == 0 &&
                              ggml_cuda_qwen_ptr_aligned(recv_flat->data, alignof(uint2)) &&
                              ggml_cuda_qwen_ptr_aligned(dst->data, alignof(uint2));
        const bool use_vec4 = use_h2 &&
                              ggml_cuda_wan_sp_vec2_recv_unpack_enabled() &&
                              (head_dim % 8) == 0 &&
                              ggml_cuda_qwen_ptr_aligned(recv_flat->data, alignof(uint4)) &&
                              ggml_cuda_qwen_ptr_aligned(dst->data, alignof(uint4));
        ...
        const bool profile = ggml_cuda_qwen_fused_profile_begin(stream, &profile_start, &profile_stop);
        if (use_vec4) {
            wan_fused_roped_all_half_recv_unpack_h2_vec4_kernel<<<blocks, threads, 0, stream>>>(
                static_cast<const uint32_t*>(recv_flat->data),
                static_cast<uint32_t*>(dst->data),
                world_size,
                qwen_params->mode == 57 ? 0 : 1,
                head_dim,
                shard_heads,
                shard_sequence,
                dst->ne[3]);
        } else if (use_vec2) {
            wan_fused_roped_all_half_recv_unpack_h2_vec2_kernel<<<blocks, threads, 0, stream>>>(
                static_cast<const uint32_t*>(recv_flat->data),
                static_cast<uint32_t*>(dst->data),
                world_size,
                qwen_params->mode == 57 ? 0 : 1,
            ...
            wan_fused_roped_all_half_recv_unpack_h2_kernel<<<blocks, threads, 0, stream>>>(
                static_cast<const uint32_t*>(recv_flat->data),
                static_cast<uint32_t*>(dst->data),
                world_size,
                qwen_params->mode == 57 ? 0 : 1,
            ...
            wan_fused_roped_all_half_recv_unpack_kernel<<<blocks, threads, 0, stream>>>(
                static_cast<const uint32_t*>(recv_flat->data),
                static_cast<half*>(dst->data),
                world_size,
                qwen_params->mode == 57 ? 0 : 1,
    ...
    if (qwen_params->mode == 59) {
        ...
        const bool use_f32_to_f16_h2 = attn->type == GGML_TYPE_F32 &&
                                       ...
                                       (attn->nb[1] % alignof(float2)) == 0 &&
                                       (attn->nb[2] % alignof(float2)) == 0;
        const int64_t total = (use_f16_h2 || use_f32_to_f16_h2) ? ggml_nelements(dst) / 2 :
                                                                  ggml_nelements(dst);
        const int threads = 256;
        const int blocks = static_cast<int>(std::min<int64_t>((total + threads - 1) / threads, 65535));
        cudaStream_t stream = ctx.stream();
        ...
        const bool profile = ggml_cuda_qwen_fused_profile_begin(stream, &profile_start, &profile_stop);
        if (use_f16_h2) {
            wan_fused_attn_head_to_seq_send_pack_f16_h2_kernel<<<blocks, threads, 0, stream>>>(
                static_cast<const char*>(attn->data),
                reinterpret_cast<half2*>(dst->data),
                world_size,
                head_dim,
            ...
            wan_fused_attn_head_to_seq_send_pack_f32_to_f16_h2_kernel<<<blocks, threads, 0, stream>>>(
                static_cast<const char*>(attn->data),
                reinterpret_cast<half2*>(dst->data),
                world_size,
                head_dim,
            ...
            wan_fused_attn_head_to_seq_send_pack_f16_kernel<<<blocks, threads, 0, stream>>>(
                static_cast<const char*>(attn->data),
                static_cast<half*>(dst->data),
                world_size,
                head_dim,
    ...
    if (qwen_params->mode == 60) {
        ...
        const bool profile = ggml_cuda_qwen_fused_profile_begin(stream, &profile_start, &profile_stop);
        if (use_h2) {
            wan_fused_attn_head_to_seq_recv_unpack_f16_h2_kernel<<<blocks, threads, 0, stream>>>(
                static_cast<const half*>(recv_flat->data),
                static_cast<float*>(dst->data),
                world_size,
                head_dim,
            ...
            wan_fused_attn_head_to_seq_recv_unpack_f16_kernel<<<blocks, threads, 0, stream>>>(
                static_cast<const half*>(recv_flat->data),
                static_cast<float*>(dst->data),
                world_size,
                head_dim,
